...
class ContrastiveSWM(nn.Module):
    ...
    def energy(self, state, next_state, no_trans=False):
        ...
        if no_trans:
            ...
            pred_trans = self.transition_model(state)
    ...
    def contrastive_loss(self, obs, next_obs):
        ...
        self.pos_loss = self.energy(state,  next_state)
        zeros = torch.zeros_like(self.pos_loss)
        
        self.pos_loss = self.pos_loss.mean()
        self.neg_loss = torch.max(
            zeros, self.hinge - self.energy(
                state, neg_state, no_trans=True)).mean()
...
class TransitionGNN(torch.nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, action_dim, num_objects,
                 ignore_action=True, copy_action=False, act_fn='relu'):
